...
singleton is shared via src.main and already loaded at import time.
"""
...
def _run_prasna_core(
    ...
    if sincerity:
        summary_parts.append(sincerity.get("message", ""))
    if house_judgment:
    ...
    if not ithasala_ok:
        ...
        if timing_estimate.get("timing_note"):
            summary_parts.append(timing_estimate["timing_note"])
...
def run_prasna_query(
    ...
    time_str: str,
    query_topic: str,
    query_house_override: int | None = None,
) -> dict:
    ...
    query_house = query_house_override or _TOPIC_TO_HOUSE[query_topic]
    errors: list[str] = []
